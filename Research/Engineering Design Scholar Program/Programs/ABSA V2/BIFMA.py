...
file_path = r'D:\OneDrive\UC Berkeley\Engineering Design Scholar Program\Programs\ABSA V2\data\BIFMA_with_BERT_Sentiment.xlsx'
...
for aspect, counts in aspect_sentiment_counts.items():
    ...
    file_path = os.path.join(output_dir, f'BIFMA_{safe_aspect}.png')
    plt.savefig(file_path)
    plt.close()

# Save results to an Excel file ((MODIFY HERE ACROSS CERTIFICATIONS)
results_path = r'D:\OneDrive\UC Berkeley\Engineering Design Scholar Program\Programs\ABSA V2\results_excels\BIFMA_with_ABSA.xlsx'
...
def extract_and_save_topics(df, aspect, sentiment):
    ...
    if not df.empty:
        ...
        try:
            ...
            # Create a valid filename by replacing any invalid characters if necessary
            # (MODIFY HERE ACROSS CERTIFICATIONS)
            topics_file_path = os.path.join(output_dir_topics, f'BIFMA_{safe_aspect}_{sentiment}_Topics.txt')
...
for aspect in aspects.keys():
    ...
    #(MODIFY HERE ACROSS CERTIFICATIONS)
    positive_file_path = os.path.join(output_dir_excels, f'BIFMA_{safe_aspect}_POSITIVE.xlsx')
    negative_file_path = os.path.join(output_dir_excels, f'BIFMA_{safe_aspect}_NEGATIVE.xlsx')
